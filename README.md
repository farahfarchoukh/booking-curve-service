...
**What's still genuinely missing**, stated plainly rather than left implicit: no database (everything is local files — fine at this scale, not at "hundreds of hotels"), no message queue / scheduler, no auth beyond a placeholder API-key header check, no automated retraining schedule, no monitoring or drift detection actually wired up (DESIGN.md §6.5/§6.6 describe what I'd build; none of it runs), no secrets management beyond a single GitHub Actions secret, no multi-stage Docker build (checked whether one would help — it wouldn't: every heavy dependency here is a prebuilt wheel, nothing compiled to shed at a build stage). `/v1/booking-curve` versioning, Prometheus `/metrics`, and in-process rate limiting (`slowapi`) *are* implemented — see `src/api.py` — but rate limiting is per-process/in-memory (no shared store across replicas). CORS is now configured (`BOOKING_CURVE_CORS_ORIGINS`, fails closed — no origins allowed by default, since this is a backend-to-backend service, not a public browser API); `tests/test_api.py::test_cors_disabled_by_default` locks in the closed default. The registry and hardened API are real, working mechanics; they were not, until this pass, an actual deployment anywhere — see "Live demo deployment" below for what changed and what's still honestly a shortcut.

**Live demo deployment.** `Dockerfile.demo` builds a self-contained variant of the service — the production `Dockerfile` deliberately has no data baked in (mounted volumes only, so the proprietary extract never touches an image that could reach a registry), so this one generates and trains on the *synthetic* demo dataset at build time instead, specifically so it can run standalone with no volume available. `.github/workflows/ci.yml`'s `publish-and-deploy-demo` job builds it, smoke-tests it exactly like the production image's job does, pushes it to `ghcr.io`, and deploys it to Cloud Run — on every merge to `main` that passes lint/test, no manual step. That's real continuous *deployment*, not just delivery. Auth is a GCP service-account key in a repo secret (`GCP_SA_KEY`) — the simpler of the two standard options; Workload Identity Federation (no long-lived key at all) is what I'd use instead for anything beyond a portfolio demo, noted rather than built since it needs its own GCP-side trust-pool setup this scale doesn't justify. The deployed service only ever runs on synthetic `hotel_X`/`hotel_Y`/`hotel_Z` data — the real Ampliphi extract has stayed off every public surface this entire project, and a live internet endpoint isn't where that boundary gets relaxed.
